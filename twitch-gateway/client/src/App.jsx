...
// ... existing code for ProfileHoverCard, SubscriptionsTable, ActiveConnectionsPanel ...
...
const LogsPanel = () => {
    ...
    return (
        <div className="bg-gray-800 rounded-lg shadow-lg border border-gray-700 mb-8">
            <div className="p-4 bg-gray-800 border-b border-gray-700 rounded-t-lg flex justify-between items-center">
                <h3 className="font-bold text-gray-200">System Logs (Last Hour)</h3>
                <button onClick={fetchLogs} className="text-xs text-indigo-400 hover:text-white font-bold uppercase"><i className="fas fa-sync mr-1"></i> Refresh</button>
            </div>
            <div className="overflow-x-auto max-h-[400px] rounded-b-lg">
                <table className="w-full text-sm text-left text-gray-400">
                    <thead className="text-xs text-gray-300 uppercase bg-gray-750 border-b border-gray-700 sticky top-0">
                        <tr>
                    ...
                    <tbody>
                        {logs.length === 0 ? (
                            <tr><td colSpan="3" className="px-4 py-4 text-center">No logs recorded.</td></tr>
                        ) : (
                            logs.map(log => (
                                <tr key={log.id} className="border-b border-gray-700 hover:bg-gray-750 last:border-0">
                                    ...
                                    <td className="px-4 py-2 text-gray-300 break-all font-mono text-xs">
                                        {log.message}
                                        {log.traceback && (
                                            <details className="mt-1">
                                                <summary className="text-[10px] text-gray-500 cursor-pointer hover:text-white">Show Traceback</summary>
                                                <pre className="mt-1 bg-black/30 p-2 rounded text-[10px] text-red-300 overflow-x-auto">
                                                    {log.traceback}
                                                </pre>
                                            </details>
                                        )}
...
const App = () => {
  ...
  // Admin View Sub-tabs
  const [adminTab, setAdminTab] = useState('dashboard'); // 'dashboard', 'logs'
  ...
  useEffect(() => {
    if(currentView === 'admin' && adminTab === 'dashboard') {
        fetchData();
                  ...
                  <div className="max-w-5xl mx-auto">
                    ...
                    <div className="flex gap-2 mb-6">
                        <button 
                            ...
                            className={`px-4 py-2 rounded font-bold text-sm transition-colors ${adminTab === 'dashboard' ? 'bg-purple-600 text-white' : 'bg-gray-800 text-gray-400 hover:text-white'}`}
                        >
                            Dashboard
                        </button>
                        <button 
                            onClick={() => setAdminTab('logs')} 
                            className={`px-4 py-2 rounded font-bold text-sm transition-colors ${adminTab === 'logs' ? 'bg-purple-600 text-white' : 'bg-gray-800 text-gray-400 hover:text-white'}`}
                        >
                            System Logs
                        </button>
                    </div>
